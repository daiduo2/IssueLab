...
class TestStreamingOutput:
    ...
    @pytest.mark.asyncio
    async def test_gqy20_multistage_judge_uses_markdown_contract(self):
        """gqy20 多阶段最终 Judge 阶段应按 Markdown 合同输出，避免 YAML 外露。"""
        from issuelab.agents.executor import _run_gqy20_multistage

        calls: list[dict[str, object]] = []

        async def fake_run_single_agent(prompt, agent_name, stage_name=None):
...
```""",
            ...
            if "当前阶段：Analyst" in prompt:
                return {"ok": True, "response": '```yaml\nsummary: "a"\n```'}
            if "当前阶段：Critic" in prompt:
                return {"ok": True, "response": '```yaml\nsummary: "c"\n```'}
            if "当前阶段：Verifier" in prompt:
                return {"ok": True, "response": '```yaml\nsummary: "v"\n```'}
...
""",
        ...
        with patch("issuelab.agents.executor.run_single_agent", side_effect=fake_run_single_agent):
            result = await _run_gqy20_multistage("agent prompt", 1, "ctx")
